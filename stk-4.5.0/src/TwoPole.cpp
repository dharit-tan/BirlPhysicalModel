...
void TwoPole :: setResonance( StkFloat frequency, StkFloat radius, bool normalize )
{
  ...
  a_[2] = radius * radius;
  a_[1] = (StkFloat) -2.0 * radius * cos(TWOPI * frequency / Stk::sampleRate());

  if ( normalize ) {
    // Normalize the filter gain ... not terribly efficient.
    StkFloat real = 1 - radius + (a_[2] - radius) * cos(TWOPI * 2 * frequency / Stk::sampleRate());
    StkFloat imag = (a_[2] - radius) * sin(TWOPI * 2 * frequency / Stk::sampleRate());
